...
we can define key, value pairs that may change the behaviour of Frappe and your
Frappe Apps.
...
performaed at bench level are generally related to this config. The config keys
defined may be used to alter behaviour of bench level operations or impact all
the sites on your bench, considering the config value precendences.
...
the key-values defined in the common config may be used as fallback in some
cases.
